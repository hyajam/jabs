...
import jabs.consensus.algorithm.Slush;
...
public class SlushNode extends PeerBlockchainNode<SnowBlock, SnowTx> {
  public static final SnowBlock SNOW_GENESIS_BLOCK = new SnowBlock(0, 0, 0, null, null);
  public SnowBlock currentBlock;

  public SlushNode(Simulator simulator, Network network, int nodeID, long downloadBandwidth, long uploadBandwidth,
      int numAllParticipants) {
    super(simulator, network, nodeID, downloadBandwidth, uploadBandwidth,
        new SnowP2P(),
        new Slush<>(new LocalBlockTree<>(SNOW_GENESIS_BLOCK), numAllParticipants));
  ...
  protected void processNewQuery(Query query) {
    ((Slush<SnowBlock, SnowTx>) this.consensusAlgorithm).newIncomingQuery(query);
  }
